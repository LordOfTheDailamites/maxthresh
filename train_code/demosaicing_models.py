...
"""
    ...
    mosaic                  = 3P x 3P x 1
"""
def demosaicing_input(submosaic, filter_size):
...
def conv_block(input_image, filter_num):
    ...
    return output_image
def demosaicer_henz(filtered_image):
    input_data = demosaicing_input(filtered_image, 8)
...
def custom_conv_block(input_image, filter_num, filter_size):
    x = tf.keras.layers.Conv2D(filters=filter_num, kernel_size=(filter_size, filter_size), padding="same")(input_image)
    x = tf.keras.layers.BatchNormalization()(x)
    output_image = tf.keras.layers.ReLU()(x)
    return output_image
    
def demosaicer_custom(filtered_image):
    conv1_1 = custom_conv_block(filtered_image, 128, 3)
    conv1_2 = custom_conv_block(conv1_1, 64, 3)
    conv1_3 = custom_conv_block(conv1_2, 3, 3)
    concat_1 = tf.keras.layers.Concatenate()([filtered_image, conv1_3])
    conv2_1 = custom_conv_block(concat_1, 128, 3)
    conv2_2 = custom_conv_block(conv2_1, 64, 3)
    conv2_3 = custom_conv_block(conv2_2, 3, 3)
    concat_2 = tf.keras.layers.Concatenate()([conv2_3, conv2_3])
    conv3_1 = custom_conv_block(concat_2, 128, 3)
    conv3_2 = custom_conv_block(conv3_1, 64, 3)
    conv3_3 = custom_conv_block(conv3_2, 3, 3)
    concat_3 = tf.keras.layers.Concatenate()([conv3_3, conv3_3])
    output_image = custom_conv_block(concat_3, 3, 3)
    return [output_image, conv1_3, conv2_3, conv3_3]