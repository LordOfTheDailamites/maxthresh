...
MODEL_ADDRESS = "..\\models_scenario_4\\cfa_demosaicer_linear_henz_bgr_8"
...
for root, dirs, files in os.walk(IMAGE_ADDRESS, topdown=True):
    for name in files:
        ...
        predictions = model.predict(image_input, verbose=1)
...
cv2.waitKey(0)
cv2.imwrite("original_image_"+str(index)+".png", predicted_images[index])
...
cv2.waitKey(0)
cv2.imwrite("linear_predicted_image"+str(index)+".png", predicted_images[index])
...
learned_filter_weights = learned_filter.get_weights()[0][0]
learned_filter_biases = learned_filter.get_weights()[1]

linear_filter = (255 * learned_filter_weights).astype("uint8")

cv2_learned_filter = cv2.resize(linear_filter, (480,480), interpolation=cv2.INTER_AREA).astype("uint8")
cv2.imshow("Learned Filter", cv2_learned_filter)
cv2.waitKey(0)

#cv2.imwrite("linear_filter_bgr_8.png", cv2_learned_filter)
np.save("linear_henz_bgr_8_filter", learned_filter_weights)
